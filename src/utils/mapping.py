...
def indexing(prj_dir, config, subject):
    
    if subject=='contig':
        abundance_dir = os.path.join(prj_dir,"Abundance")
        job_dir = os.path.join(abundance_dir,"jobs")
        log_dir = os.path.join(abundance_dir,"logs")
        out_dir = os.path.join(abundance_dir,"out")
        source_fasta = os.path.join(prj_dir,'OVU','rep_contigs.fasta')
        fasta = os.path.join(abundance_dir, "rep_contigs.fasta")
        faidx = os.path.join(abundance_dir,"rep_contigs.fa.fai")
        gtf = os.path.join(abundance_dir,"rep_contigs.gtf")
    elif subject=='gene':
        abundance_dir = os.path.join(prj_dir,'proteins',"gene_abundance")
        job_dir = os.path.join(abundance_dir,"jobs")
        log_dir = os.path.join(abundance_dir,"logs")
        out_dir = os.path.join(abundance_dir,"out")
        source_fasta = os.path.join(prj_dir,'proteins','gene_clusering','rep_genes.fna')
        fasta = os.path.join(abundance_dir,"rep_genes.fna")
        faidx = os.path.join(abundance_dir,"rep_genes.fa.fai")
        gtf = os.path.join(abundance_dir,"rep_genes.gtf")
    ...
    bash_commands = [
        ...
        f"cp {source_fasta} {abundance_dir}/",
...
def mapping(prj_dir, manifest, config, subject):
    
    df = pd.read_csv(manifest, header=0, names=["fileHeader", "fq1", "fq2"], index_col=None)
    ...
    for chunk in chunks:
        ...
        bash_commands = [
            ...
            f"\tfasta={fasta}",
            f"\tgtf={gtf}",
            f"\tout_dir={out_dir}/$header",
            f"\tbam={out_dir}/$header/\"$header\"_sorted.bam",
            f"\tstat={out_dir}/$header/\"$header\"_stat.txt",
            f"\tcount={out_dir}/$header/\"$header\"_count.tsv",
            f"\tmkdir -p $out_dir",
...
def check(abundance_dir, manifest):
    manifest = pd.read_csv(manifest, header=0, names=["fileHeader", "fq1", "fq2"], index_col=None)
    ...
    def check_single(s, abundance_dir):
        columns_to_check = ["_count.tsv", "_count.tsv.summary", "_sorted.bam", "_sorted.bam.bai", "_stat.txt"]
        for col in columns_to_check:
            s[col] = os.path.isfile(os.path.join(abundance_dir, "out", s["fileHeader"], s["fileHeader"]+col))
        if s[columns_to_check].all():
            s["mapping"] = True
        return s
    mapping = mapping.apply(lambda s: check_single(s, abundance_dir=abundance_dir), axis=1)
    mapping.to_csv(os.path.join(abundance_dir, "mapping_check.csv"),index=None)
    return mapping

def count_matrix(prj_dir, manifest, subject):
    if subject=='contig':
        abundance_dir = os.path.join(prj_dir,"Abundance")
        job_dir = os.path.join(abundance_dir,"jobs")
        log_dir = os.path.join(abundance_dir,"logs")
    ...
    def get_all_count(prj_dir, manifest):
        mapping_check = check(abundance_dir=abundance_dir, manifest=manifest)
        ...
        for idx, row in mapping_check.iterrows():
            if row["mapping"]==False:
                ...
                current_count = pd.read_table(
                    os.path.join(out_dir, row["fileHeader"], f"{row['fileHeader']}_count.tsv"),header=1,
                ...
                print(idx, row["fileHeader"], 'processed.')
        return all_count
    ...
    all_count.to_csv(os.path.join(abundance_dir, "all_count.csv"), index=None)
    ...
    FPKM.to_csv(os.path.join(abundance_dir,"all_FPKM.csv"), index=None)
    TPM.to_csv(os.path.join(abundance_dir,"all_TPM.csv"), index=None)
