...
def summarise_OVUs(prj_dir, include=["CAT", "VCT", "GNM"]):
    ...
    def incldue_vContact3(OVU_info_tmp_path, reps_lineage_path):
        ...
        vcontact3 = vcontact3_summary.loc[vcontact3_summary["Reference"]==False, :].drop("index",axis=1).reset_index(drop=True).rename({
        ...
        # clean CAT lineage, replace "no support"/"nan" with empty string
        ...
        # keep the first part of the lineage if there are multiple parts seperated by "|"
    ...
    def length_info(OVU_info, quality_filtered_contigs_path):
        # 先构建 contig_id 到长度的字典，加速查找
        contig_length_dict = {}
        with open(quality_filtered_contigs_path, 'r') as f:
            for record in SeqIO.parse(f, "fasta"):
                contig_length_dict[record.id] = len(record.seq)

        def get_lengths(contigs_str):
            contigs = contigs_str.split(",")
            return [contig_length_dict.get(cid, 0) for cid in contigs]

        # 直接用列表推导式批量处理
        lengths_list = OVU_info["contigs_in_cluster"].map(get_lengths)
        OVU_info["cluster_length"] = lengths_list.map(sum)
        OVU_info["cluster_median_length"] = lengths_list.map(lambda x: np.median(x) if x else 0)
        OVU_info["cluster_mean_length"] = OVU_info["cluster_length"] / OVU_info["cluster_size"]
        OVU_info = OVU_info.loc[:, ['OVU', 'representative_contig', 'cluster_size', 'cluster_length', 'cluster_median_length', 'cluster_mean_length', 'lineage', 'contigs_in_cluster']]
        return OVU_info
    ...
    reps_lineage = reps_lineage.loc[:,["OVU", "representative_contig","cluster_size","lineage","CAT_lineage","vContact3_lineage","GeNomad_lineage","contigs_in_cluster"]]
    reps_lineage.to_csv(reps_lineage_path, index=None)
